...
export interface CombatShopEntry {
  ...
  def: ActionCardDef | TacticCardDef;
  goldPrice: number;
  crystalPrice: number;
}
...
export function priceFor(def: ActionCardDef | TacticCardDef): { gold: number; crystals: number } {
  const baseGold = ACTION_GOLD[def.rarity] ?? 150;
  const baseCrystal = ACTION_CRYSTAL[def.rarity] ?? 10;
  const ratio = def.type === 'tactic' ? TACTIC_RATIO : 1;
  return {
...
export function rollCombatShop(dayKey: string, rerollCount: number = 0): CombatShopEntry[] {
  ...
  const entries: CombatShopEntry[] = [];
  for (let i = 0; i < SHOP_SLOT_COUNT; i++) {
    // 70% Action, 30% Tactic (matches GDD's 30:20 catalog ratio plus a
    // mild Action bias since Actions drive combat).
    const isAction = rng.next() < 0.7;
    const pool: ReadonlyArray<ActionCardDef | TacticCardDef> = isAction ? actions : tactics;
    const filtered = pickByRarityWeights(pool, rng);
    if (!filtered) continue;
    const price = priceFor(filtered);
    entries.push({
      kind: isAction ? 'action' : 'tactic',
      cardId: filtered.id,
      def: filtered,
